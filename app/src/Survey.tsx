    ...
    <section aria-labelledby={`survey-${s.id}`}>
      ...
      {open && (
        <div className="survey-who">
          ...
          {/**
            * **화면이 거짓말을 하면 안 된다.**
            * 처음에는 「화면 어디에도 보이지 않습니다」 라고 적었는데, 운영자 화면에서
            * 누가 무엇을 골랐는지 볼 수 있게 되면서 그 문장이 사실이 아니게 됐다.
            * 그래서 「운영진만 확인합니다」 로 고쳐 두었다.
            *
            * 2026-08-24 에 DB 가 또 바뀌었다 — 응답 행에 이름·구역번호를 안 남기고
            * 되돌릴 수 없게 섞은 열쇠만 남긴다(202608240001a). 운영자 화면에도 「익명」 으로만
            * 나온다. 그래서 문구를 다시 고친다. **DB 를 바꿀 때 이 줄도 같이 본다.**
            *
            * 「아무도 모른다」 라고는 안 적는다. 명부가 21명뿐이라 DB 를 통째로 볼 수 있는
            * 사람은 하나씩 넣어 맞춰 볼 수 있다. 적을 수 있는 참말은 「운영진도 모른다」 까지다.
            */}
          <p className="survey-who-note">
            <b>단톡방 프로필과 같게</b> 적어 주세요.
            {' '}회원인지 확인하고 두 번 세지 않으려고만 씁니다 — 이름은 저장하지 않아
            {' '}<b>운영진도 누가 무엇을 골랐는지 모릅니다.</b>
          </p>
          {!locked && (
