...
export type GoogleSurveyRound = {
  ...
  period: string
  /**
   * 몇 명이 답했나. 총원과 미응답자 수는 적지 않는다.
   * 화면에는 `20명 응답` 으로 뜬다 — **사람을 세는 단위는 「명」 이다.**
   * (`분` 은 높임말이라 숫자 뒤에 붙이면 시간 단위와 헷갈린다.)
   */
  answered: number
  /** 「2026. 9. 2. 기준」 — 결과 페이지의 배지와 같은 값 */
  asOf: string
...
export const GOOGLE_SURVEYS: GoogleSurveyRound[] = [
  {
    ...
    /**
     * **네 번 늘었다.** 처음 집계한 8월 26일에는 17건이었는데, 운영규정 동의 공지
     * (마감 8월 31일) 뒤로 8월 31일·9월 1일·9월 2일에 더 들어와 21건이 됐다.
     * 그중 한 사람이 두 번 낸 것을 빼서 **유효 20명**이다. 총원 21명이므로 미응답은 한 명뿐이다.
     *
     * 여기 숫자는 운영자 화면의 회차 카드에 그대로 뜬다. 분석 가이드는 20명으로
     * 세는데 이 카드가 다른 수를 말하면 **같은 화면 안에서 두 값이 어긋난다.**
     * 아래 백분율도 그때 다시 센 값이다 — 함께 고치지 않으면 카드만 옛말을 한다.
     */
    answered: 20,
    asOf: '2026. 9. 2. 기준 · 중복 1건 제외',
    resultUrl: './survey-result.html',
    highlights: [
      { question: '언제가 좋은가', answer: '토요일 오후', percent: 80 },
      { question: '어디가 편한가', answer: '서울 도심 · 종로 · 중구', percent: 75 },
      { question: '무엇에 관심이 있는가', answer: '미술 전시 · 갤러리', percent: 70 },
      { question: '고를 때 무엇이 중요한가', answer: '작품 · 전시의 내용과 완성도', percent: 70 },
    ],
