...
export const TENTATIVE: Tentative[] = [
  {
    id: 'spain-exhibition-casual',
    tag: '일정 미정',
    // 9월 관람 전시 투표 공동 2위(5표 · 202608240001b_september_poll.sql:19).
    // 톡방 2026-08-26 09:07 에 "2위작 스페인과 정원은 따로 벙개로 올리려고 합니다" 로
    // 예고됐고, 저장소의 마지막 메시지(2026-09-01 18:02)까지 후속 언급이 없다.
    //
    // 톡방에는 《스페인의 거장 고야》 라는 **다른 전시**도 나온다(2026-08-18).
    // 그쪽은 동호회 일정으로 거론된 적이 없으니 헷갈리지 말 것.
    text: '9월 관람 전시 투표에서 공동 2위에 오른 《스페인 미술 500년: 빛과 어둠의 연대기》를 '
      + '따로 벙개로 보기로 했습니다. 날짜와 모집 공지는 아직 정해지지 않았습니다.',
    // 후보 날짜가 나온 적이 없다. 타입 주석대로 이 배열은 「다 지났는지」 검사가
    // 보는 자리이므로, 없는 날짜를 지어 넣지 않는다.
    candidates: [],
  },
]
