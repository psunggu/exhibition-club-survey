...
export const MEETUPS: Meetup[] = withDefaults([
  ...
  // 다녀왔다 — 2026-08-30 에 완료로 옮겼다. 8월 29일 관람일이 지났다.
  // 주석은 반드시 이 자리(덩어리 밖)에 둔다. id…status 사이에 끼우면
  // validate-meetup-taxonomy.mjs 의 정규식이 이 모임을 통째로 못 읽는다.
  {
    id: 'gaudi-visit',
    ...
    // **집결하지 않았다.** 운영자가 2026-08-27 에 개별 참석으로 정했다.
    // 그래서 「시간 확인 중」 이 아니었고, 완료 줄에도 **참석 인원이 없다** —
    // 모이지 않았으니 셀 자리가 없었다. 다른 완료 모임이 「참석 11명」 을 적는 그 자리에
    // 「개별 참석」 이라고 적어, 빠뜨린 것이 아니라 없는 것임을 밝힌다.
    time: '개별 관람',
    venue: '신사하우스 · 신사동',
    description: '가우디 서거 100주기를 맞아 원본 작품과 유물, 공식 공인 레플리카로 그의 창작 세계를 살펴보는 전시입니다.',
    // 「각자 다녀오셔도 된다」 고 권하는 문장이라 휴관일을 함께 적는다.
    // 2026-09-03 에 공식 예매 페이지(feverup.com/m/665616)에서 확인했다 —
    // 기간 8/1~10/31, **추석 당일 9/25 휴관**. 2026년 추석 연휴는 9/24(목)~9/26(토)라
    // 회원이 각자 다녀오기 좋은 날이고, 모르면 헛걸음한다.
    note: '개별 참석으로 진행해 따로 집결하지 않았고, 그래서 참석 인원은 따로 세지 않았습니다. '
      + '전시는 10월 31일까지 열려 있으니 아직 못 가신 분은 각자 편한 때에 다녀오셔도 됩니다. '
      + '추석 당일(9/25)은 휴관입니다.',
    infoUrl: 'https://feverup.com/m/665616',
    infoLabel: '전시·예매 정보 보기 →',
    mapUrl: 'https://map.kakao.com/?q=%EC%8B%A0%EC%82%AC%ED%95%98%EC%9A%B0%EC%8A%A4',
    completedRow: '8/29 (토) 가우디 서울전 관람 · 신사하우스 · 개별 참석'
  },
  {
    id: 'today-garden-visit',
    date: '2026-09-01',
    chip: '관람 완료',
    kind: 'done',
    regular: false,
    ...
    status: '완료',
    tone: 'done',
    title: '조은 《오늘의 정원》 수시 관람',
    dateLabel: '2026. 9. 1. (화)',
    time: '오후 2시',
    venue: '그라운드시소 이스트 · 2층 매표소 앞',
    // 9월 관람 전시 투표에서 공동 2위였다(5표 · 202608240001b_september_poll.sql:73).
    // 1위 서도호는 정기관람(9/19)으로 가고, 2위 둘은 따로 벙개로 보기로 했다 —
    // 그 가운데 이쪽이 평일에 먼저 이뤄졌다. 나머지 하나(스페인)는 아래 TENTATIVE 에 있다.
    // '정기관람' 이라는 낱말을 쓰지 않는다 — 분류 검사가 이 필드도 읽어서,
    // 그 글자가 있으면 regular 가 true 여야 한다고 잡는다. 이 모임은 수시다.
    description: '9월 관람 전시 투표에서 공동 2위에 오른 전시입니다. 1위작은 9월 공식 모임이 되었고, '
      + '2위작 둘은 따로 벙개로 보기로 해 그 가운데 하나를 평일에 다녀왔습니다.',
    note: '사전 예매 후 관람을 마쳤습니다. 개인별 이름은 공개하지 않습니다.',
    // infoUrl 을 비워 둔다. 예매 링크 후보가 하나 있었으나 그것이 이 전시의
    // 예매 페이지인지 확인하지 못했다 — **확인 안 된 링크는 없는 것보다 나쁘다.**
    infoUrl: '',
    infoLabel: '',
    mapUrl: 'https://map.kakao.com/?q=%EA%B7%B8%EB%9D%BC%EC%9A%B4%EB%93%9C%EC%8B%9C%EC%86%8C%20%EC%9D%B4%EC%8A%A4%ED%8A%B8',
    completedRow: '9/1 (화) 14:00 조은 《오늘의 정원》 관람 · 그라운드시소 이스트'
  },
  {
    id: 'carmen-membership',
    date: '2026-09-12',
    chip: '카르멘 17:00',
    kind: 'conf',
    regular: false,
    ...
    time: '오후 5시 · 90분 · 인터미션 없음',
    venue: '예술의전당 콘서트홀',
    description: '경기필하모닉오케스트라와 성악가들이 비제의 오페라 《카르멘》 주요 장면을 들려주는 전석 초대 공연입니다.',
    // 단체 관람이 아니다. 회원음악회 초대권 두 장이 나서 쓸 사람을 정한 건이고,
  ...
  },
  {
    ...
    /**
     * 토요일이라 저녁 관람이 가능하다. 서울관은 **수·토만 21시까지** 열고
     * 나머지 요일은 18시에 닫는다 — 다른 요일을 골랐으면 이 시간표가 성립하지 않는다.
     * 관람일 투표가 토요일로 몰린 것과 맞물린 결과다.
     *
     * **집결을 15:50, 관람을 16시로 당겼다 (2026-09-10 운영자 결정).** 투표 항목은
     * 「17-18관람」 이었는데 미술관이 회차 입장이라 **16시 회차를 예매한 회원**이 있었다
     * (정리봇 9/1~9/7). 17시 집결이면 그분들과 어긋나므로 16시 회차에 맞춰 10분 전에
     * 모이고, 식사 시간(18시)은 투표대로 둔다 — 관람이 두 시간이 되는 셈인데,
     * 전시실이 넓어 60분은 빠듯했다. 처음엔 「16:00 집결」 로 적었다가 같은 날 15:50 으로 고쳤다.
     */
    time: '15:50 집결 · 16:00~18:00 관람 · 18:00~ 식사·티타임',
    /**
    ...
    description: '이주와 거주, 개인과 공동체, 공간과 기억을 탐구해 온 설치미술가 서도호의 '
      ...
      + '15시 50분에 모여 16시에 입장해 관람하고, 18시부터 꽃누리들밥 경복궁점에서 저녁 식사와 티타임이 이어집니다.',
    /**
     * **예매 안내를 맨 앞에 둔다.** 이 모임에서 회원이 놓칠 수 있는 것은
     * 집결 시각이 아니라 티켓이다 — 각자 예매해야 하고, 매진되면 못 온다.
     * 신청자 이름은 적지 않는다 (AGENTS.md 「실제 회원 데이터」). **인원도 적지 않는다** —
     * 2026-09-10 운영자 결정. 취합 중인 숫자를 카드에 두면 낡은 채로 남는다(9/3 의 「10명」 이 그랬다).
     */
    /*
     * 「한 아이디로 4매」 와 「양도 금지」 를 넣은 이유 (2026-09-03 공식 안내 재확인).
     *
     * 신청이 10명인데 그 둘을 모르면 한 분이 몰아서 잡으려다 4매에서 막히고,
     * 대신 잡아 준 표는 예약자 본인이 아니라 입장이 거절될 수 있다.
     * 「각자 예매」 라고만 적어 두면 왜 그래야 하는지가 없어 그대로 읽히지 않는다.
     *
     * 식사 장소는 2026-09-09 운영자가 꽃누리들밥 경복궁점으로 정했다. 그전(09-03)에는
     * 「재조정될 수 있습니다」 였다 — 정해진 적이 없는 것과 정해도 바뀔 수 있는 것은 다르다.
     * **인원은 어떤 것도 적지 않는다** — 식사 인원도, 티켓 신청 수도(2026-09-10 운영자 결정).
     * 요약 카드(meetingBrief.ts)도 같은 날 숫자를 전부 뺐다. 인원은 톡방에서 취합한다.
     */
    note: '티켓은 각자 예매하셔야 합니다. 예매는 9월 7일(월)부터 열려 있고 '
      + '매진이 빠르니 아직이면 지금 잡아 주세요 — 16시 회차로 맞추면 함께 들어갑니다. '
      + '한 아이디로 최대 4매까지만 되고, '
      + '관람권 양도는 금지라 예약자 본인이 아니면 입장이 막힐 수 있습니다. '
      + '온라인 예매가 어려우면 현장 선착순 구매도 됩니다. '
      + '관람료는 8,000원입니다. 예매를 마치면 톡방 공지에 댓글로 알려 주세요. '
      + '만 24세 이하·학부생 등은 증빙하면 무료라고 공식 안내에 있으니 해당되면 확인해 보세요. '
      + '관람 뒤 식사·티타임은 꽃누리들밥 경복궁점입니다.',
  ...
  },
  {
    ...
    tone: 'conf',
    title: '경복궁 야간관람',
    dateLabel: '2026. 10. 2. (금)',
    time: '오후 7시~9시 30분 · 입장 마감 오후 8시 30분',
    venue: '경복궁 · 서울 종로구 사직로 161',
    description: '해설사가 동행하는 별빛야행이 아니라, 2026년 하반기 경복궁 야간 자유관람 회차입니다.',
    /*
     * 상태를 「확정 · 예매분 한정」 으로 적은 이유.
     *
     * 동호회 공식 일정으로 공지된 적이 **없다.** 1인 예매 한도가 4매라 그만큼만
     * 미리 잡혔고, 나머지는 각자 예매해야 한다. 그러니 「우리 동호회가 간다」 는
     * 뜻의 그냥 「확정」 은 아니다.
     *
     * 그렇다고 점선(`tent`)으로 두지도 않았다. 확보된 좌석에 대해서는 날짜·시각이
     * 이미 고정돼 있어 「아직 안 정해졌다」 도 사실이 아니기 때문이다.
     *
     * 갈림길은 **표 없는 회원이 현장에 가는 것**을 막는 것이다. 그래서 상태 글자와
     * 아래 note 가 「예매한 표가 있어야 한다」 고 분명히 말한다. 추가 참석은 각자
     * 예매해야 하고 마감이 10/1 이라 그 뒤에는 방법이 없다.
     *
     * `venueKind` 는 '박물관' 이다. '모임' 은 킥오프처럼 볼 것이 없는 자리에 쓰는 값이라,
     * 달별 목록이 「수시 · 모임」 으로 아무것도 말하지 않게 된다. 궁궐 관람은 '박물관' 이 가깝다.
     *
     * 같은 이유로 `chip` 에 **조건**을 함께 적었다. 달력 격자에서 보이는 것은 칩 하나뿐이라
     * 「야간관람 19시」 로 두면 10/2 칸만 본 회원이 표 없이 간다. 칸이 좁아 넷을 다 못 담으므로
     * 「관람」 을 덜어 시각과 조건을 남겼다 — 「야간」 만으로도 무엇인지 읽히고,
     * 제목·시각 전체는 바로 아래 달별 목록과 상세 팝업에 있다.
     * (주석을 `id`~`status` 사이에 끼우지 말 것 — validate-meetup-taxonomy 가
     *  그 일곱 줄을 연속으로 읽어서 모임 하나를 통째로 놓친다.)
     *
     * note 에 「누가 신청했다 밀렸다」 를 적지 않는다 — 이 페이지는 공개고 21명
     * 동아리라 당사자가 자기 얘기로 읽는다. 회원이 **행동할 정보**만 남긴다.
     */
    note: '경복궁 야간 자유관람은 예매한 표가 있어야 입장할 수 있습니다. 미리 확보된 4석은 모두 찼습니다. '
 ...
 * (2026-08-29 부터 2026-09-03 까지 실제로 비어 있었다.)
 */
export const TENTATIVE: Tentative[] = [
  /*
   * **스페인전은 여기서 뺐다** (2026-09-05).
   *
   * 9월 관람 전시 투표 공동 2위였고 「따로 벙개로 올리겠다」 고 예고됐지만,
   * 운영자가 **동호회 일정으로 잡지 않고 각자 보는 것**으로 정했다.
   * 그러면 「조율 중 · 미정」 에 둘 수 없다 — 그 절은 **날짜를 맞추는 중**인 것을
   * 담는 자리고, 조율할 것이 없는 줄을 거기 두면 회원이 기다리게 된다.
   *
   * 전시 자체는 **보드에 있다** — 《스페인 미술 500년》, 예술의전당 한가람디자인미술관,
   * 2026-09-22 ~ 2027-01-20. 보고 싶은 사람은 거기서 찾는다.
   *
...
/**
 * **관람일이 지나면 완료다 — 손으로 옮기지 않는다.**
 *
 * 예전에는 모임이 끝날 때마다 사람이 `kind` 를 'conf' → 'done' 으로 바꾸고
 * `completedRow` 를 적어야 했다. 잊으면 그 모임은 **완료 목록에도 예정 목록에도
 * 없이 조용히 사라졌다** — 검사기가 14일 뒤에야 빨간불로 알려 줬다.
 *
 * 날짜는 이미 자료에 있다. 거기서 뽑으면 잊을 일이 없다.
 *
 * `dead` 는 제외한다 — 예매 마감일 같은 줄이라 **다녀올 것이 없다.**
 * `today` 를 인자로 받는 이유는 `monthsToShow` 와 같다.
 */
export function isDone(m: Meetup, today: string): boolean {
  if (m.kind === 'dead' || m.kind === 'tent') return false
  return m.kind === 'done' || m.date < today
}

/**
 * 화면이 쓸 갈래. **지난 `conf` 는 `done` 으로 보인다** — 달력 칩 색이 따라온다.
 * 자료의 `kind` 는 그대로 둔다. 고쳐 쓰면 「손으로 적은 것」 과 「날짜에서 나온 것」 이
 * 섞여, 다음에 읽는 사람이 어느 쪽인지 알 수 없게 된다.
 */
