...
const EXPECTED_GONE = [
  ...
  /*
   * ── 아래 둘은 디자인이 아니라 **일정이 지나서** 없어진 글이다 ──────
   * 가우디 서울전(8/29)을 2026-08-30 에 완료로 옮겼다. 고정본 notice.html 은
   * 그 모임이 아직 예정이던 때에 묶여 있어 영영 「가우디 확정」 이라고 말한다.
   *
   * **글이 사라진 것이 아니라 자리를 옮겼다.** 달력 칸은 「관람 완료」 로 바뀌었고,
   * 「이번 달 모임」 목록과 「완료된 모임」 의 완료 줄에 그대로 있다 —
   * 「8/29 (토) 가우디 서울전 관람 · 신사하우스 · 개별 참석」.
   *
   * 앞으로도 모임이 끝날 때마다 이런 줄이 하나씩 생긴다. 그때마다 여기 적는 대신
   * 「완료되면 다 넘어간다」 는 넓은 규칙을 쓰고 싶어지는데, **그러면 안 된다** —
   * 이 검사가 막으려는 것이 바로 「모임이 조용히 사라지는 것」 이기 때문이다.
   * 한 건씩 좁게 적어서, 사람이 한 번은 눈으로 보게 한다.
   */
  {
    re: /^가우디 확정$/,
    why: '가우디 서울전(8/29)이 끝나 2026-08-30 에 완료로 옮겼다. 달력 칸의 딱지가 '
      + '「가우디 확정」 에서 회색 「관람 완료」 로 바뀐 것이다. 고정본은 예정이던 때에 묶여 있다.',
  },
  {
    re: /^개별 관람 · 신사하우스\(신사동\)$/,
    why: '위와 같다. 「다가오는 확정 모임」 에 있던 가우디 카드가 통째로 완료 쪽으로 옮겨 가면서 '
      + '그 카드의 시간·장소 줄도 함께 옮겼다. 완료 줄에 「신사하우스 · 개별 참석」 으로 남아 있다.',
  },
]
