 ...
 * `4133/홍길동` 같은 **구역번호 + 실명**이 줄마다 들어 있다.
 ...
 * (`4133/홍길동` 꼴). 거기서 성명과 이름을 모아 두었다가 본문에서도 지운다.
 *
 * 이게 필요한 이유: 호칭 규칙만으로는 샌다. 실제로 `길동 형제님` 이
...
function scrub(s) {
  ...
  // 오픈채팅 프로필 규칙: 4133/홍길동
  t = t.replace(/\b\d{3,4}\s*\/\s*[가-힣]{2,4}\b/g, () => { count('구역+이름'); return '[회원]'; });
  // 호칭이 붙은 실명. `형제·자매` 를 빠뜨렸다가 `길동 형제님` 이 새어 나갔다.
  ...
  // `홍길동` 이 `길동` 으로 먼저 잘려 반쪽만 남는 일이 없다.
