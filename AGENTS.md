...
> 최종 갱신: 2026-09-03 (달력 격자 자동화 · #83 머지 · 운영 DB 적용은 남음)
...
## 다음 작업 후보 (우선순위 순 · 2026-09-03 갱신)

1. **#83 의 SQL 을 운영 DB 에 적용한다 (보안 · 최우선 · 사람이 해야 한다)**

   **PR 은 2026-09-03 에 머지됐다**(`600dc93`). 그런데 **머지로 닫히는 것이 아니다** —
   바뀐 것은 `supabase/migrations/` 의 파일이고, 마이그레이션은 저장소에 있다고
   실행되지 않는다. **운영자가 Supabase SQL Editor 에서 돌려야** 실제로 닫힌다.
   그때까지는 열려 있다.

   돌릴 것 둘, 순서대로:

   1. `202608280003a_admin_gate_hardening.sql` — 그대로 붙여 넣는다. 자리표시자가 없다.
      확인 질의 둘이 딸려 있다(휘발성이 `v` 인지, 남은 함수가 0줄인지).
   2. `202608200001d_admin_password.template.sql` — **자리표시자를 채워** 돌린다.
      bcrypt 비용이 6 → 10 이 됐는데 **이미 저장된 해시는 6 그대로다.** 검증은 해시에
      박힌 비용을 쓰므로, **운영자 세 분이 암호를 다시 정해야** 10 이 적용된다.
      채운 파일은 저장소에 저장하지 않는다.

   앱은 영향받지 않는다 — `app/src/lib/survey.ts:303` 에서 모든 rpc 가 POST 한 곳을 거친다.
   막는 것은 화면이 쓰지 않는 길이다.

   **이 저장소는 public 이다.** 결함의 재현 경로를 여기에 적지 않는다 — 적어 두면
   고치기 전까지 안내문이 된다. 무엇을 왜 바꾸는지는 머지된 마이그레이션 파일 머리말에 있다.

   같이 열려 있던 나머지는 정리됐다 — #102 는 머지(`26fba14`), #27 · #18 은
   `exhibition_club_codex_package/` 가 사라져 대상 경로가 없으므로 2026-09-02 에 닫았다.
   **2026-09-03 기준 열린 PR 은 없다.**
2. **보드 「최종 정보 업데이트」 날짜** — 세 곳(`App.tsx` 의 `SITE_INFO_UPDATED_ON`,
