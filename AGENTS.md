...
1. **달력 격자 자동화** — `MONTHS` · `PAST_MONTHS` 가 손으로 박혀 있어 **10월이 되면 달력이
   8·9월에 멈춘다.** 「완료된 모임」 목록은 이미 날짜에서 뽑아 자동으로 도니, 같은 방식으로
   격자도 뽑는다(이번 달·다음 달을 펼치고 지난 달은 접는다). 기한이 있는 유일한 항목이다.
2. **열린 PR 넷 정리** — #83(운영자 암호가 URL 에 실리지 않게·맞히기 지연, **보안**),
   #102(운영자 목록에서 다녀온 설문 접기 — `SurveyAdmin.tsx` 를 건드려 충돌 가능),
   #27(캐시 갱신), #18(운영 문서). 오래 열려 있을수록 `main` 과 멀어진다.
