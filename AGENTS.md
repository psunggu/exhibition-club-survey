...
2. **#83 을 처리한다 (보안)** — 운영자 암호가 URL 에 실리지 않게 하고 맞히기를 늦춘다.
   `survey_admin_ok` · `survey_admin_members` 가 `stable` 이라 PostgREST 가 GET 을 받는다.
   암호가 URL 쿼리에 실리고, `survey_admin_members` 는 교인 명부를 통째로 돌려준다.
   **2026-09-02 확인: main 에서 두 함수 모두 아직 `stable` 이고 이를 바꾼 마이그레이션이
   없다 — 결함이 살아 있다.** `scripts/validate-survey-schema.mjs` 와
   `202608200001d_admin_password.template.sql` 에 충돌이 있어 갱신해야 머지된다.
