...
- 영화 순위는 소유자 맥의 launchd(`scripts/update-movies-task.sh`, 등록 `scripts/install-launchd.sh movies`)가 PR 로 올려 머지한다. Windows 용 `*.ps1` 은 2026-09-24 이관 전 원본으로 남겨 둔다. GitHub 러너는 KOBIS 에 못 붙는다. ruleset 에 우회 대상을 만들지 않는다.
