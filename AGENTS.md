...
> 최종 갱신: 2026-07-22 (Claude Code 세션에서 작성)
...
2. **모임 일정 공지 페이지** — `public/notice.html` + `notice.css` + `notice.js` (2026-07-21 신규 추가). 카카오톡 단체방 대화를 정리해 만든 7·8월 모임 공지. 달력의 "오늘" 마커는 notice.js가 접속 시점 날짜로 동적 표시(달력 제목 "YYYY년 M월"과 `.dnum` 텍스트 매칭). 단톡방에 URL이 공유되어 회원들이 수시로 열람 — 방장이 공지 일정표에 등록함. 아래 "notice 페이지" 절 참고.
...
### 현재 반영된 일정 (2026-07-22 20:18 기준)

- 확정: 7/26(일) 성률 기획전 관람 (3부 예배 후, 김혜정·송종민·엄영순) / 7/29(수) 19:00 퐁피두센터 한화 〈큐비스트〉 평일 관람 (문화의날 14,000원)
- 날짜 조율 중: 8월 정기관람 서울역사박물관 — 당초 8/8(토) 15시 공지였으나 희망일정 투표에서 8/22(토) 우세, 확정 공지 대기 (퀴즈 이벤트, 담당 김혜정)
- 완료: 7/5 킥오프, 7/11 큐비스트 주말 관람
- 미정: 7/29 오전 벙개, 가우디 서울전(얼리버드 7/31 마감), 자율 관람

## 다음 작업 후보 (우선순위 순)

1. **8월 박물관 최종 날짜 반영** — 투표에서 8/22(토) 우세. 톡게시판 최종 공지가 나오면 notice.html의 카드(현재 "날짜 조율 중")와 달력 칩(8/8 "조율중", 8/22 "유력")을 확정 상태로 갱신.
