...
3. **운영자 화면에는 설문 관리도 회원 명부도 없다**(2026-09-10). `#/survey/admin` 은 보드 소식 · 구글 설문 분석만 다룬다. 설문 관리와 명부 코드는 `selfSurvey: true` 로 켜면 돌아오지만, 켤 이유가 생기기 전에는 켜지 않는다. **명부 데이터는 쓰지 않는 개인정보라 지운다** — 운영자가 SQL Editor 에서 `delete from public.survey_members;` (되돌릴 수 없다. 다시 필요하면 `202608270001c_members_bulk.template.sql` 로 넣는다).
