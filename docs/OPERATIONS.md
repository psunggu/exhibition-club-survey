...
git commit -am "정리봇 M월 D일 ~ M월 D일" && git push -u origin HEAD && gh pr create --fill && gh pr merge --squash --auto
```
...
npm run check:quick
git commit -am "보드 영화 순위를 M월 D일 기준으로 갱신한다" && git push -u origin HEAD && gh pr create --fill && gh pr merge --squash --auto
```
...
git commit -am "10월 정기관람을 달력에 올린다" && git push -u origin HEAD && gh pr create --fill && gh pr merge --squash --auto
```
...
```bash
git push -u origin HEAD && gh pr create --fill && gh pr merge --squash --auto
```
