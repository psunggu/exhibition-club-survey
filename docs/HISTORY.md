...
- 스크롤: **눌러서 옮기면 새 화면 맨 위, 돌아오면(뒤로 · 앞으로 · 새로 고침 · 설문 결과 페이지에서) 떠날 때의 자리.** 브라우저의 자리 되살리기는 이 앱에서 믿을 수 없었다 — 해시로 뒤로 가면 옛 화면이 아직 그려져 있을 때 되살려 짧은 쪽에 걸렸고(일정 1500px → 1066px, 보드는 목록을 새로 받느라 맨 위), 띠가 붙은 뒤 새로 고침은 1548px · 0px 로 들쭉날쭉했다. 그래서 끄고(`manual`) `lib/scrollMemory.ts` 가 기록 칸마다 이름표를 달아 자리를 적는다(메모리 + sessionStorage, 이름표와 숫자뿐). 돌아오면 페이지가 자라는 동안 다시 맞추고, 회원이 움직이면 멈춘다. 링크 하나만 기억하던 첫 판은 앞으로 가기 · 새로 고침에서 맨 위로 떨어졌다(코드 검토에서 잡았다). 지금 칸을 다시 누르면 맨 위로 간다.
