...
- 2026-09-25 — **맥 이관 마무리.** 스킬(`/ops` · `/digest` · `/meetup`) · `OPERATIONS.md` · `AUTOMATION_PLAN.md` · `CLAUDE.md` 에 남았던 `C:\` 경로 · PowerShell · 작업 스케줄러 절차를 launchd · `~/KakaoDigest/inbox` 기준으로 바꿨다. 맥은 내보내기를 사람이 하는 반자동이 설계라(카톡 메뉴가 자주 바뀐다) 윈도우의 「자동 내보내기 복구」 는 할 일에서 뺐다. `/digest` 는 이제 배치와 같은 `digest-public-task.sh` 를 부른다 — 손으로 올린 기간을 배치가 다시 연 일(#179 뒤 #183)은 손 절차가 상태 파일을 안 거쳐서였다. 같은 이유로 배치는 `origin/main` 공개본의 `period_label` 이 같으면 PR 을 열지 않는다(상태 `already-on-main`). `npm run notice` 는 맥에서 `pbcopy` 로 클립보드에 넣는다 — launchd 처럼 로캘이 비어도 한글이 안 깨지게 `LC_ALL` 을 넘긴다.
- 2026-09-24 — **배치를 Windows PC 에서 맥으로 옮겼다.** 작업 스케줄러 + `*.ps1` 자리에 launchd + `*.sh`(`update-movies-task.sh` · `recheck-task.sh` · `digest-public-task.sh`, 등록 `install-launchd.sh <작업>`). 절차 · 로그 · 상태 파일 · 실패 시 main 되돌리기 · 「원격에 올라간 브랜치는 남긴다」 는 ps1 과 같게 옮겼고 ps1 은 원본으로 남긴다. 풍선은 맥 알림, 메모장은 `open`. cron 이 아니라 launchd 인 이유는 잠자던 시각을 깨어난 뒤 한 번 돌려 주기 때문이다(`StartWhenAvailable` 과 같은 효과). 화면 기준(`screen-baseline.json`)은 다음 `screens:save` 부터 `darwin` 에서 찍힌다.
- 2026-09-14 — 자동화 준비물 1·2 (`docs/AUTOMATION_PLAN.md`). **재확인 배치** `scripts/recheck-task.ps1`(월 06:00): `recheck --exit-on-change` 가 바뀐 페이지가 있을 때만 종료 3 을 내고, 그때만 헤드리스 `claude -p`(sonnet)에 스킬의 견주기·보고 절과 자료를 넣어 표를 받는다 — 변화 없는 주는 토큰 0. 첫 실제 실행에서 헤드리스 호출이 터미널 CLI 로그인 만료로 실패했는데 배치는 자료·알림·상태를 남기고 종료 3 으로 끝났다(의도한 열화). **정리봇 공개본 배치** `scripts/digest-public-task.ps1`(매일 06:30): 새 `digest-*.json` 이 있을 때만 변환·검사·PR 까지, 머지는 사람. 바뀜 판정은 공개본 JSON 으로만 한다 — `screens:save` 가 같은 내용에도 기준 파일을 다시 써서(실측 11줄) 전체 `git status` 로 보면 매일 빈 PR 이 열린다. 새로 만든 `.ps1` 넷이 BOM 없이 저장되어 PS 5.1 파서가 한글 근처 따옴표를 삼켰다 — BOM 을 붙여 해결. 등록(`install-*-task.ps1`)은 운영자가 한다.
- 2026-09-13 — 정기 배치 점검. 영화 순위 배치는 세 번 중 한 번(9/12 13:55) KOBIS 연결 시간 초과 한 번으로 통째로 실패했고, 그 뒤 체크아웃이 `content/movies-…` 에 남아 있었다. 05:00 에 PC 가 자고 있어 배치가 깨어나는 아무 때나(낮 포함) 도는 것이 원인의 절반 — 낮의 KOBIS 는 느리다. 받기를 시도마다 30초 · 세 번(5초 · 15초 간격)으로 하고 재시도 사실을 로그에 남기며(`scripts/fetch-retry.mjs`), 실패하면 `main` 으로 되돌리고 push 안 한 브랜치는 지운다. 카톡 수집 배치는 스케줄 실행 2/2 실패(방 창 없음) — 방 창을 트리거로 바꾸는 안은 운영자가 더 보고 정하기로 했다. 백업 배치는 정상.
- 2026-09-10 (셋째) — 남은 잔재를 걷어 냈다. **회원 명부**는 「사이트 응답 자격 확인」 과 「운영진 답하기」 를 위한 것이었는데 둘 다 껐으므로 화면에서 빼고(같은 스위치), DB 행은 쓰지 않는 개인정보라 운영자가 SQL 로 지운다. 회원 화면의 **「일자·시간」 「운영·요청」 탭**은 사이트 투표가 없으면 새 내용이 생기지 않는 죽은 탭이라 꺼진 설정에서 그리지 않고(`visibleTabs()`), 옛 주소는 관람 장소로 보낸다. **제목**은 「… 설문」 → 「… 투표 결과」(`categoryHeading()`), 「지난 설문」 → 「지난 투표」 — 달력·보드와 말을 맞췄다. 검사기 두 곳의 꺼진 설정 절에 그만큼을 더했다.
- 2026-09-10 — 운영자 화면에서도 **설문 만들기·고치기·지우기를 뺐다.** 운영자 화면 사용성 검토에서 「운영자가 매달 하는 일(표 수 옮기기)이 SQL 이라 화면 밖에 있다」 는 것이 1순위로 나왔는데, 운영자는 그 자리를 만드는 대신 **투표도 결과 공유도 전부 톡방에서** 하기로 했다. 사이트는 정해진 것만 요약 카드에 적는다. 회원 응답 화면과 같은 스위치(`selfSurvey`)로 끄고 코드는 남겼다 — 켜면 그대로 돌아온다. `validate-survey-admin-ui` 는 켠 설정으로 설문 관리를 계속 재고, 끝에 꺼진 설정 절을 더해 「단추와 목록이 없다 · 소식·명부·구글은 있다」 를 잰다.
...
## 사이트 띠 — 모임 일정 · 관람 정보 · 투표 (2026-09-26)

- 운영자 요청으로 모든 화면 맨 위에 세 칸짜리 띠를 둔다. 그전에는 화면마다 가는 길이 달랐다 — 보드는 우상단 링크 둘, 일정은 보드 카드 하나, 투표는 「모임 일정 보기」 하나. 그래서 **일정에서 투표로, 투표에서 보드로 가는 길이 없었다.** 이름과 순서는 운영자가 정했다(「관람 정보」 는 새 이름이고 화면 제목은 「문화 콘텐츠 공유 보드」 그대로다).
- 모양은 시안 열한 가지(상단 · 하단 · 폴더 · 그림 · 번호 · 두 줄 설명 등)를 회원 경험 · 디자인 규칙 · 구현 위험 세 관점으로 매겨 **잉크 띠 색인**으로 정했다. 잉크 바탕에 지금 칸만 종이색으로 칠하고 아랫변을 열어 페이지와 잇는다 — 보드의 지역 · 유형 탭과 투표 알약(모두 종이 위의 닫힌 모양)과 층이 갈린다. 하단 고정 막대는 카톡 인앱 아래 도구줄과 겹쳐 뺐다.
- 휴대폰 글씨를 ×1.6 · ×2 로 키워 재 보니 시안의 고정 높이(쉬는 칸 44 · 지금 칸 48)가 칸마다 줄 수가 달라질 때 지금 칸을 14–24px 가라앉혔고, px 줄 높이를 쓴 시안은 글자가 겹쳤다. 그래서 줄은 stretch 로 맞추고 줄 높이는 비율로 적는다. 좁은 화면(≤551px)에서 좌우 여백을 줄여 360px 안드로이드는 ×1.6 까지 한 줄로 든다.
- 스크롤: **눌러서 옮기면 새 화면 맨 위, 돌아오면(뒤로 · 앞으로 · 새로 고침 · 설문 결과 페이지에서) 떠날 때의 자리.** 브라우저의 자리 되살리기는 이 앱에서 믿을 수 없었다 — 해시로 뒤로 가면 옛 화면이 아직 그려져 있을 때 되살려 짧은 쪽에 걸렸고(일정 1500px → 1066px, 보드는 목록을 새로 받느라 맨 위), 띠가 붙은 뒤 새로 고침은 1548px · 0px 로 들쭉날쭉했다. 그래서 끄고(`manual`) `lib/scrollMemory.ts` 가 기록 칸마다 이름표를 달아 자리를 적는다(메모리 + sessionStorage, 이름표와 숫자뿐). 돌아오면 페이지가 자라는 동안 다시 맞추고, 회원이 움직이면 멈춘다. 링크 하나만 기억하던 첫 판은 앞으로 가기 · 새로 고침에서 맨 위로 떨어졌다(코드 검토에서 잡았다). 되살리기는 **글꼴이 다 오고 잠잠할 때까지** 다시 맞춘다 — 한글 시스템 글꼴이 없는 리눅스 CI 에서 대체 글꼴로 먼저 맞춘 뒤 Pretendard 가 와 높이가 196px 줄며 자리가 밀렸다. **보드의 지역 · 유형 · 검색어도 칸마다 기억한다** — 자리만 되살리면 필터가 기본값으로 돌아가 엉뚱한 목록의 한가운데에 섰다. 되살리기는 한 번에 하나만 돈다(앞 되살리기가 뒤로 가기 뒤까지 남아 앞 화면을 끌고 간 일). 지금 칸을 다시 누르면 맨 위로 간다.
- 초점 여백은 `html` 의 `scroll-padding` 이 아니라 본문 초점 요소의 `scroll-margin` 에 준다(띠의 실제 높이 `--site-nav-h` + 8). `html` 에 주면 띠 자신의 칸도 가려졌다고 읽혀 칸 사이를 Tab 으로 옮길 때마다 페이지가 굴렀다.
- **두 단계로 냈다.** 1단계(#197 · #198)는 띠만 더하고 우상단 링크 · 일정의 보드 카드 · 투표의 「모임 일정 보기」 는 남겼다(「회원이 누르던 자리」 를 지킨다는 예전 결정을 바꾸는 일이라). 운영자가 배포된 화면을 보고 같은 날 셋을 걷게 했다 — 「상단의 탭으로 이동하면 된다」. 일정의 보드 카드는 계획처럼 아래로 옮기지 않고 **없앴다.** 운영자 화면의 「투표 결과 화면으로」 는 띠에서 켜지는 칸이 없어 남긴다.
- 검사는 `validate-site-nav` 한곳에 모았다 — 주소마다 켜지는 칸, 큰 글씨, 붙어 있기, 누르기 · 뒤로 가기, 세 화면에서 같은 모양. 화면 대조는 `position` 을 재지 않아 띠가 붙지 않게 돼도 통과하기 때문이다.

## 지난 것은 날짜로 저절로 (2026-09-26)

- 운영자 요청 — 「투표 탭에서 일정이 지난 9월 정기모임 요약은 지난 투표로 빼 달라」, 「이후에도 일정이 지난 모임이나 투표는 지난 것으로 분류해 달라」.
- 모임(달력)은 이미 날짜로 갈렸다(`isDone`). 투표는 **이어진 모임이 있을 때만** 모임 날짜로 내려갔고, 이어진 모임이 없으면 영영 남았다(「짝을 지어내지 않는다」). 요약 카드는 하나만 두고 갈아 끼워, 모임이 지나도 맨 위에 남다가 다음 요약이 들어오면 **통째로 사라질** 참이었다.
- 바꾼 것: 요약에 `meetupId` 를 달아 모임 날짜가 지나면 「지난 투표」 맨 앞에 접힌 줄로 내려간다(열면 같은 카드). 요약은 `BRIEFS` 배열로 — 새 모임을 맨 앞에 더하고 지난 것을 지우지 않는다. 이어진 모임이 없는 투표는 마감 30일 뒤 지난 것으로 본다 — 짝은 여전히 지어내지 않고 마감일만 센다. 마감 직후는 결과를 보러 오는 때라 바로 접지 않는다.
- `validate-survey-ui` 가 진짜 시각으로 재고 있어서, 9/19 모임이 지나자 요약 절 열두 곳이 한꺼번에 깨졌다(요약이 맨 위에서 빠졌으므로). 브라우저 시계와 가짜 자료의 「지금」 을 **첫 요약 모임의 전날 낮**(소스에서 읽는다)으로 묶고, 모임 다음 날로 옮겨 「지난 투표」 로 내려가는지를 따로 잰다.

## 검사용 정적 서버 (2026-09-27)

- 사이트 띠 작업 뒤 보안 점검에서 나왔다. 화면 검사기 일곱이 `dist/` 를 띄우는 서버를 저마다 베껴 두었는데, 주소 없이 `listen(port)` 해 **모든 네트워크에** 열렸고, 풀어 낸 경로를 그대로 `path.join` 해 `..%2f` 로 **`dist/` 밖 파일**(`~/.ssh` 까지)을 내줬다. 소유자 맥은 방화벽이 꺼져 있어, `npm run check` 가 도는 2분 동안 같은 와이파이의 기기가 읽을 수 있었다. 배포된 사이트와는 상관없다.
- `scripts/static-server.mjs` 하나로 모았다 — 127.0.0.1 에서만 받고, 루트 밖 · 잘못된 `%` · NUL 은 404. 새 검사기 `validate-site-nav` 가 옛 서버를 또 베낀 것이 계기라, 한 곳만 고치면 나머지가 그대로 남는다는 것이 이유다.
