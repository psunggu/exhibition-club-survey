...
-- 한 글자만 다르게 적어도 지금은 **다른 사람으로 세어진다.**
--
-- 여기에 구역번호와 이름을 나란히 쓴 예시를 두지 않는다.
-- 그 모양 자체가 개인정보 검사에 걸리는 꼴이고, 실제로 이 주석이 한 번 걸렸다.
-- (이 저장소는 공개다. 실제 회원 이름은 어디에도 적지 않는다.)
--
-- ── 이 표는 잠근다 ─────────────────────────────────────────
...
-- 구역번호는 숫자만 남긴다 — `4133번` 이나 뒤에 `구역` 을 붙여 적어도 같은 것으로 본다.
--
-- (숫자 뒤에 한글을 바로 붙여 적은 예시를 여기 두지 않는다. 그 모양이 곧
--  개인정보 검사가 찾는 꼴이고, 이 주석이 실제로 거기 걸렸다.)
--
...
--
-- **셋. 실패 문구는 화면과 서버가 같아야 한다.**
-- 한동안 「이 설문에 응답할 수 없습니다」 로 뭉갰다. 캐내는 사람에게 힌트가 될까 해서였다.
-- 그런데 뭉개도 감춰지는 것이 없었다 — 거절당했다는 사실 자체가 이미 답이다.
-- 정작 회원만 왜 안 되는지 모른 채 되돌아갔다. 그래서 그대로 말한다.
-- 감추는 일은 문구가 아니라 위의 횟수 제한이 한다.
